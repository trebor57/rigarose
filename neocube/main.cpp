...
WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, INT nCmdShow)
{
	...
	try
	{
		...
		if(GetPrivateProfileString("server", "grf_file", NULL, settings.szGrf, sizeof(settings.szGrf) / sizeof(settings.szGrf[0]), INIFILE) <= 0)
			if(GetPrivateProfileString("server", "vfs_file", NULL, settings.szVfs, sizeof(settings.szVfs) / sizeof(settings.szGrf[0]), INIFILE) <= 0)
				throw "Invalid key in NeonCube.ini: grf_file&vfs_file";
	...
	try
	{
		...
		int grfExist = CheckFileForExistance(settings.szGrf);
		int vfsExist = CheckFileForExistance(settings.szVfs);
		int switchVal = 0;
		if(grfExist && vfsExist)
			if(grfExist)
				switchVal = grfExist;
			else
				switchVal = vfsExist;
		...
		{
			case CFFE_FILE_NOT_FOUND:
				throw "Invalid entry in neoncube.ini: \"grf_file\" or \"vfs_file\" (file not found)";
				break;

			case CFFE_PATH_NOT_FOUND:
				throw "Invalid entry in neoncube.ini: \"grf_file\" or \"vfs_file\" (invalid path)";
				break;

			case CFFE_ACCESS_DENIED:
				throw "Invalid entry in neoncube.ini \"grf_file\" or \"vfs_file\" (access denied)";
				break;

			case CFFE_UNKNOWN_ERROR:
				throw "Invalid entry in neoncube.ini \"grf_file\" or \"vfs_file\" (unknown error)";
				break;
		}
...
// otherwise "GRF" it'll be repacked into the grf. "VFS" for add to VFS.
//
...
int ProcessPatchLine(GrfCache *pCache, LOCALPATCHITEMCONTAINER *pItems, const std::pair<std::string /*patchName*/, int /*patchDest*/> &patchMetaPair, bool &isDirty)
{
	...
	case 1:  // GRF
		{
			...
			else if (lstrcmpiA(ext, "rar") == 0)
			{
				doExtractRar = true;
				extractRarForGrf = false;
				extractRarForVfs = true;
			}
		}
		break;
		//------------------------------
		//- VFS Packing (by JC_Master) -
		//------------------------------
		case 2:  // VFS
		{
			if ((lstrcmpiA(ext, "gpf") == 0) || (lstrcmpiA(ext, "grf") == 0))
			{
				//NOT IMPLEMENTED
				PostError(false, "Trying to unpack gpf or grf in VFS !");
			}
			else if (lstrcmpiA(ext, "rar") == 0)
			{
				doExtractRar = true;
	...
	case 10:  // <del>VFS
		{
			//Delet File
			ctrl.getVfsCtrl().RemoveFile(patchMetaPair.first);
		}
		break;
	}
	...
	if ( doExtractRar )
	{
		...
		if (!extractRarForGrf && !extractRarForVfs)
		{
		...
		while((RHCode = RARReadHeader(hData, &HeaderData)) == 0)
		{
			...
			//- VFS Packing (by JC_Master) -
			//------------------------------
			if(extractRarForVfs && (HeaderData.Flags & 0xE0) != 0xE0 )  // not a directory
			{
				//Add to VFS and delete
				char extractedRelativeName[MAX_PATH];
				PathCombineA(extractedRelativeName, extractRarTargetDestination, HeaderData.FileName);
				
				if(!ctrl.getVfsCtrl().AddFile(ctrl.getVfsName().c_str(), extractedRelativeName, HeaderData.FileName))
					PostError(true, "Fatal error : can't compact file \"%s\" in \"%s\" from \"%s\" !", HeaderData.FileName, ctrl.getVfsName().c_str(), extractedRelativeName);
				
				char baseDir[MAX_PATH];
				DeleteFileA(extractedRelativeName);

				lstrcpynA(baseDir, extractedRelativeName, MAX_PATH);
				baseDir[MAX_PATH - 1] = 0;
				PathRemoveFileSpecA(baseDir);
				char FindExpr[MAX_PATH];
				PathCombine(FindExpr, baseDir, "*");
		...
		//Delete .rar!
		DeleteFileA(OpenArchiveData.ArcName);
	}
